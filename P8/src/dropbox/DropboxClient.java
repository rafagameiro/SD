...
public class DropboxClient
{
	...
	private static final String accessTokenStr = "WmV0QM_iQOAAAAAAAAAAKj_NJP1ybpaPeUwbEsqEejkJsWkEwUMcd7UypNQRSAd0";
	...
	public Result<byte[]> download(String filename) {
		try {			
			AccessFileV2Args file = new AccessFileV2Args(filename);
			...
			downloadFile.addHeader(DROPBOX_API_ARG, JSON.encode(file));
			
			service.signRequest(accessToken, downloadFile);
			Response r = service.execute(downloadFile);
			
//			OAuthRequest downloadFileLocal = new OAuthRequest(Verb.POST, GET_TEMPORARY_LINK_FILE_V2_URL);
//			downloadFileLocal.addHeader("Content-Type", JSON_CONTENT_TYPE);
//			downloadFileLocal.setPayload(JSON.encode(file));
//			
//			service.signRequest(accessToken, downloadFileLocal);
//			Response rLocal = service.execute(downloadFileLocal);
